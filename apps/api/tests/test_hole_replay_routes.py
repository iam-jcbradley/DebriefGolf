...
def _seed_round_with_hole_geometry() -> tuple[int, int]:
    """One hole with real tee/green/shot geometry. Returns (round_id, hole_number)."""
    with Session(engine) as session:
        user = User(email=f"test-replay-{uuid.uuid4()}@example.com", name="Test User")
...
def test_list_round_holes_reports_par_yardage_and_shot_count() -> None:
    round_id, hole_number = _seed_round_with_hole_geometry()
...
def test_list_round_holes_empty_for_course_less_round() -> None:
    with Session(engine) as session:
        user = User(email=f"test-replay-nocourse-{uuid.uuid4()}@example.com", name="Test User")
        session.add(user)
        session.commit()
        session.refresh(user)
...
def test_list_round_holes_404_for_unknown_round() -> None:
    response = client.get("/api/rounds/999999/holes")
    assert response.status_code == 404


def test_hole_replay_includes_geometry_and_shots() -> None:
    round_id, hole_number = _seed_round_with_hole_geometry()
...
def test_hole_replay_reports_short_sided_count() -> None:
    round_id, hole_number = _seed_round_with_hole_geometry()
...
def test_hole_replay_404_for_unknown_hole_number() -> None:
    round_id, _ = _seed_round_with_hole_geometry()
    response = client.get(f"/api/rounds/{round_id}/holes/99/replay")
    assert response.status_code == 404


def test_hole_replay_409_for_course_less_round() -> None:
    with Session(engine) as session:
        user = User(email=f"test-replay-nocourse2-{uuid.uuid4()}@example.com", name="Test User")
        session.add(user)
        session.commit()
        session.refresh(user)
        round_ = Round(user_id=user.id, status=RoundStatus.needs_audit)
...
def test_hole_replay_404_for_unknown_round() -> None:
