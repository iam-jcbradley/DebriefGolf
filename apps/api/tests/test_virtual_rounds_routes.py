...
def _seed_user(session: Session, email: str = "virtual@example.com") -> int:
...
class TestCreateVirtualRound:
    def test_creates_a_virtual_round(self, client: TestClient, db_session: Session) -> None:
    ...
    def test_rejects_unknown_platform(self, client: TestClient, db_session: Session) -> None:
        user_id = _seed_user(db_session)
        response = client.post(
            "/api/virtual-rounds",
...
class TestListAndGetVirtualRounds:
    def test_list_filters_by_user_id(self, client: TestClient, db_session: Session) -> None:
        user_a = _seed_user(db_session, "player-a@example.com")
        user_b = _seed_user(db_session, "player-b@example.com")
        client.post(
            "/api/virtual-rounds",
    ...
    def test_get_404s_for_unknown_id(self, client: TestClient) -> None:
